...
- **高亮渲染禁止改正文 DOM**：一律走 CSS Custom Highlight API（`useReaderHighlights` + `::highlight(yudu-hl-*)`，`index.css` 双主题规则），不要往正文插 `<mark>`——会破坏多栏分页测量与 React 受控渲染；正文重排后需 `notifyLayout` 重建 Range
- 选区偏移 ↔ DOM Range 换算统一走 `lib/textAnchor.ts`（TreeWalker 口径），正文容器以 `data-hl-chapter` 标记
